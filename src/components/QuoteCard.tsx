...
const QuoteCard = forwardRef<HTMLDivElement, QuoteCardProps>(
  ({ quote, mood, name, date, template, isStoryMode }, ref) => {
    ...
    const aspectClass = isStoryMode ? 'aspect-[9/16] w-[360px]' : 'aspect-[4/5] w-full max-w-[420px]';

    if (template === 'classic') {
      return (
        <div
          ref={ref}
          className={`${aspectClass} rounded-2xl p-8 flex flex-col justify-between relative overflow-hidden`}
          style={baseStyle}
        >
          ...
          <div className="relative z-10 flex-1 flex items-center justify-center">
            <div className="text-center">
              <div className="text-5xl mb-6">{moodInfo.emoji}</div>
              <p className="font-display text-2xl leading-relaxed tracking-wide">
                「{quote}」
              </p>
            </div>
          </div>
          <div className="relative z-10 text-center space-y-1">
            {name && (
              <p className="font-handwriting text-lg" style={{ color: subTextColor }}>
                ── 給 {name} 的金句
              </p>
            )}
            <p className="font-serif-tc text-sm" style={{ color: subTextColor }}>{date}</p>
    ...
    if (template === 'modern') {
      return (
        <div
          ref={ref}
          className={`${aspectClass} rounded-3xl p-8 flex flex-col justify-between relative overflow-hidden`}
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 w-32 h-32 rounded-full opacity-15" style={{
            background: 'rgba(255,255,255,0.3)',
            filter: 'blur(30px)',
            transform: 'translate(-20%, 20%)',
          }} />
          ...
          <div className="relative z-10 flex-1 flex items-center">
            <p className="font-display text-3xl leading-loose tracking-wider">
              {quote}
            </p>
          </div>
          <div className="relative z-10 flex items-end justify-between">
            <div>
              {name && <p className="font-handwriting text-base" style={{ color: subTextColor }}>致 {name}</p>}
    ...
    return (
      <div
        ref={ref}
        className={`${aspectClass} rounded-xl p-10 flex flex-col justify-center relative border-2`}
      ...
      >
        <div className="text-center space-y-8">
          ...
          <p className="font-display text-2xl leading-relaxed" style={{ color: isDark ? '#faf8f5' : '#2c1810' }}>
            「{quote}」
          </p>
          <div className="space-y-1">
            {name && (
              <p className="font-handwriting text-base" style={{ color: subTextColor }}>── {name}</p>
            )}
